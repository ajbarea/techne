...
2. **Commit each group, in order.** For each group in `COMMITS.md`:
   ...
   - **Never add a "🤖 Generated with [Claude Code](https://claude.com/claude-code)" footer** (or any variant — "Generated by Claude", emoji-prefixed attribution, etc.). This mirrors the usual global rule: no assistant attribution trailers in commit history. Strip it from any HEREDOC template before the `git commit` runs. Same rule applies to the PR body in step 4.
