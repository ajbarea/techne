# Sisters audit: the checks

Run from the shell after the config preamble in `SKILL.md`, which sets `$SISTERS`,
`$TEAM_SISTERS`, `$WORKSPACE` and `$GITHUB_USER`. Every check is read-only.

## Contents

- [1. Action-pin drift](#1-action-pin-drift)
...
- [9. Codecov config presence + bot-comment silencing](#9-codecov-config-presence--bot-comment-silencing)
...
For every workflow file (`.github/workflows/*.yml` and `*.yaml`) across the active sisters, extract `uses: <actor>/<action>@<version>` pins. Most steps are written `- uses: …`, so the pattern must allow the list dash; without it the check sees a fraction of the pins and reports "no drift" over the rest.

```
for repo in $SISTERS; do
  grep -hE '^\s*(-\s+)?uses:' $WORKSPACE/$repo/.github/workflows/*.y*ml 2>/dev/null \
    | sed -E 's/^\s*(-\s+)?uses:\s*//; s/\s*$//' \
...
```
for repo in $SISTERS; do
  git -C "$WORKSPACE/$repo" fetch --quiet origin
  behind=$(git -C "$WORKSPACE/$repo" rev-list --count main..origin/main 2>/dev/null)
  ahead=$(git -C "$WORKSPACE/$repo" rev-list --count origin/main..main 2>/dev/null)
...
- **Python-script-delegated clean**: `LOG_ARCHIVE_MAX_AGE_DAYS = N` constant in `scripts/clean_build.py` or `scripts/dev.py`.
